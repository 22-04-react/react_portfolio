...
/*
  takeLatest : 액션 요청이 여러번 들어오면 제일 최근 요청 하나만 실행 (takeEvery: 들어오는 요청 모두처리)
  all : 여러개의 함수를 한번에 호출
  call : 특정함수를 동기적으로 호출 (api요청시 주로 사용, 두번째 인수값으로 받은 내용을 첫번째 인수로 받은 함수에 적용)
  fork : 특정함수를 비동기적으로 호출
  put : 리듀서에 액션객체를 전달 (dispatch랑 동일)
*/
